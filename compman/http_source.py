...
class _AuthAwareRedirectHandler(HTTPRedirectHandler):
    """Redirect handler that drops the configured auth header off-host.
    ...
    def redirect_request(
        ...
        if target_host is None or target_host.lower() != req.host.lower():
            new_request.headers = {
                key: value
...
def fetch(
    ...
    if auth is not None:
        ...
        response = opener.open(Request(url, headers={auth.header: value}), timeout=30)
    else:
