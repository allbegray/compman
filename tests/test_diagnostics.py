...
def _write_s3_backup_project(path: Path) -> None:
    (path / "compman.yml").write_text(
        "compman:\n"
        "  name: test-app\n"
        "  dirs:\n"
...
def test_backup_store_check_ok_with_credentials_and_region(tmp_path, monkeypatch, dummy_runtime):
    _write_s3_backup_project(tmp_path)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("AWS_ACCESS_KEY_ID", "key")
    monkeypatch.setenv("AWS_SECRET_ACCESS_KEY", "secret")
    monkeypatch.setenv("AWS_DEFAULT_REGION", "ap-northeast-2")
    ...
    check = next(item for item in report.checks if item.id == "backup_store")
    assert check.severity == "warning"
    assert check.ok is True
    assert "s3://bucket/backups" in check.message


def test_backup_store_check_reports_missing_credentials(tmp_path, monkeypatch, dummy_runtime):
    _write_s3_backup_project(tmp_path)
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("AWS_ACCESS_KEY_ID", raising=False)
    monkeypatch.delenv("AWS_SECRET_ACCESS_KEY", raising=False)
...
def test_backup_store_check_reports_missing_region(tmp_path, monkeypatch, dummy_runtime):
    _write_s3_backup_project(tmp_path)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("AWS_ACCESS_KEY_ID", "key")
    monkeypatch.setenv("AWS_SECRET_ACCESS_KEY", "secret")
    ...
    assert any(entry["id"] == "backup_store" for entry in json_checks)
